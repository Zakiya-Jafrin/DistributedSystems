...
//			sendHeartbeats();
			...
			String request = UDPMulticastServer.ReceiveUDPMessage(Constants.PORT_SE_TO_RM);//"";//"100000001 MTLM1234 " + Constants.OP_ADD_EVENT + " MTLE100519 S 2";
...
//			String request = "100000007 MTLM1234 " + Constants.OP_GET_BOOKING_SCHEDULE + " A";
			...
			while (request != "") {
				...
				case Constants.OP_ADD_EVENT:
					if (city.addEvent(user.getID(), request_parts[3], request_parts[4], Integer.valueOf(request_parts[5]))) {
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,true," + "'Event " + request_parts[3] + " added successfully.'", Constants.IP_RM2, Constants.PORT_RMs);
					} else {
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,false," + "'Error adding event " + request_parts[3] + "..'", Constants.IP_RM2, Constants.PORT_RMs);
					}
					break;
				case Constants.OP_REMOVE_EVENT:
					if (city.removeEvent(user.getID(), request_parts[3], request_parts[4])) {
						UDPMulticastClient.SendUDPMessage(
								request_parts[0] + ",RM2,true," + "'Event " + request_parts[3] + " removed successfully'", Constants.IP_RM2, Constants.PORT_RMs);
					} else {
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,false," + "'Error removing event " + request_parts[3] + "'", Constants.IP_RM2, Constants.PORT_RMs);
					}
					break;
				case Constants.OP_LIST_EVENT_AVAILABILITY:
					...
					UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,true," + events, Constants.IP_RM2, Constants.PORT_RMs);
					break;
				case Constants.OP_BOOK_EVENT:
					result = (city.bookEvent(user.getID(), request_parts[3], request_parts[4]));
					if (Boolean.valueOf(result[0]))
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,true," + "'Event (" + request_parts[3] + ") booked successfully'", Constants.IP_RM2, Constants.PORT_RMs);
					else
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,false," + result[1], Constants.IP_RM2, Constants.PORT_RMs);
					break;
				case Constants.OP_CANCEL_EVENT:
					result = (city.cancelEvent(user.getID(), request_parts[3], request_parts[4]));
					if (Boolean.valueOf(result[0]))
						UDPMulticastClient.SendUDPMessage(
								request_parts[0] + ",RM2,true," + "Event (" + request_parts[3] + ") successfully cancelled", Constants.IP_RM2, Constants.PORT_RMs);
					else
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,false," + result[1], Constants.IP_RM2, Constants.PORT_RMs);
					break;
				case Constants.OP_SWAP_EVENT:
					result = (city.swapEvent(user.getID(), request_parts[3], request_parts[4], request_parts[5],
							request_parts[6]));
					if (Boolean.valueOf(result[0]))
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,true," + "Event (" + request_parts[3] + ") swapped successfully", Constants.IP_RM2, Constants.PORT_RMs);
					else
						UDPMulticastClient.SendUDPMessage(request_parts[0] + ",RM2,false," + result[1], Constants.IP_RM2, Constants.PORT_RMs);
					break;
	        ...
	        public void run() {
	        	try
	        	{
		        	DateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
 		            UDPMulticastClient.SendUDPMessage("RM2... Sending heartbeat at : " + dateformat.format(new Date()) +
		              " \\r\\n Thread's name: " + Thread.currentThread().getName(), Constants.IP_RM2, Constants.PORT_RMs);	
 		            //UDPMulticastClient.SendUDPMessage("finish", Constants.IP_RM2, 1234);
	        	}
	        	catch(Exception e)
	        	{
